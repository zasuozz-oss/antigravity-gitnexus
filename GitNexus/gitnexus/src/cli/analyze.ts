...
export interface AnalyzeOptions {
  ...
  skills?: boolean;
  verbose?: boolean;
  /** Custom ignore filter (from Unity preset etc.). Passed through to pipeline. */
...
/**
 * Analyze a project with a custom ignore filter.
 * Called by `gitnexus unity analyze` to inject Unity preset rules.
 */
export const analyzeWithFilter = async (
  repoPath: string,
  ignoreFilter: { ignored: (p: any) => boolean; childrenIgnored: (p: any) => boolean },
  options?: { force?: boolean; embeddings?: boolean; verbose?: boolean },
) => {
  return analyzeCommand(repoPath, {
    ...options,
    ignoreFilter,
  });
};
