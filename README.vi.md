...
1. **Cài đặt** `gitnexus` globally qua `npm install -g gitnexus@latest`
2. **Cấu hình** Antigravity MCP (`~/.gemini/antigravity/mcp_config.json`)
3. **Cài đặt** `gitnexus-sync` vào `~/.local/bin/` — đồng bộ skill GitNexus sang định dạng Antigravity
4. **Clone** repo GitNexus (qua `gh fork` hoặc `git clone`) và cài Web UI dependencies
...
```
gitnexus-setup/
├── setup.sh          # Setup chính — cài global, MCP config, cài sync, clone Web UI
...
Script cài gitnexus globally và cấu hình `~/.gemini/antigravity/mcp_config.json`:
...
Dùng lệnh `gitnexus` cài global thay vì `npx` để tránh bug npm v11 arborist với git dependency (`tree-sitter-dart`). Để cập nhật: `npm install -g gitnexus@latest`.
